...
<template>
  <div class="w-full h-fit grid items-center sm:grid-cols-2 md:grid-cols-3 gap-x-6 gap-y-3">
    <a :href="content.link" v-for="content in array" :key="content.id"
      ...
      data-aos="fade-up">
      <div
        class="min-w-full min-h-full bg-gradient-to-t from-black opacity-50 hover:opacity-70 transition-opacity duration-100 z-20">
      </div>
